...
# --- ИСПРАВЛЕННЫЙ СИСТЕМНЫЙ ПРОМПТ ДЛЯ ИИ ---
SYSTEM_PROMPT = """
...
3. **МЕТАФОРМУЛА (самое важное):**
   ...
   - **ВНИМАНИЕ: Проверяй грамматику! Фраза должна быть грамматически правильной на русском языке**
...
---
## 🔑 Ваша Метаформула
### [ЗДЕСЬ НАПИШИ ФРАЗУ-КОД КАПСЛОКОМ КРУПНО]
        ...
        try:
            ...
            if hasattr(response, 'choices') and len(response.choices) > 0:
                ...
                if hasattr(choice, 'message') and hasattr(choice.message, 'content'):
                    ...
                    import re
                    # Ищем метаформулу в отчете
                    formula_pattern = r"### \[(.*?)\]"
                    match = re.search(formula_pattern, content, re.IGNORECASE | re.DOTALL)
                    if match:
                        formula = match.group(1).strip()
                        # Исправляем грамматические ошибки
                        corrected_formula = correct_formula_grammar(formula)
                        if corrected_formula != formula:
                            # Заменяем исправленную формулу в отчете
                            content = content.replace(formula, corrected_formula)
...
def correct_formula_grammar(formula: str) -> str:
