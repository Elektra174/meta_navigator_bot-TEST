...
# --- ОБНОВЛЕННЫЙ СПИСОК ВОПРОСОВ (8 ШАГОВ) ---
QUESTIONS = [
    ...
    # Шаг 3: Поиск утечки энергии (Логический мост)
    ...
    # Шаг 5: Поиск датчика в теле (Экспертная подсказка)
...
async def start_audit_flow(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.answer(
            ...
            f"В течение следующих {len(QUESTIONS)} шагов мы проведем диагностику ваших внутренних настроек. "
            "Отвечайте максимально честно — это поможет собрать вашу личную Метаформулу.",
            parse_mode="Markdown"
        )
        
        await asyncio.sleep(1)
        # Отправляем первый вопрос с его заголовком
        await callback.message.answer(f"Шаг 1: Обнаружение затыка\n\n{QUESTIONS[0]}")
...
async def process_answer(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if next_step < len(QUESTIONS):
            ...
            # Добавляем названия шагов для каждого вопроса
            step_titles = [
                "Обнаружение затыка",
                "Моделирование системы без сбоя",
                "Поиск утечки энергии",
                "Внешний образ проблемы",
                "Поиск датчика в теле",
                "Скан скрытой силы (Тень)",
                "Расчет цены бездействия",
                "Команда на перепрошивку"
            ]
            
            title = step_titles[next_step] if next_step < len(step_titles) else f"Шаг {next_step + 1}"
            await message.answer(f"Шаг {next_step + 1}: {title}\n\n{QUESTIONS[next_step]}")
        else:
