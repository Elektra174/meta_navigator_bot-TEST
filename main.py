...
# --- УЛУЧШЕННЫЙ СИСТЕМНЫЙ ПРОМПТ v3.1 ---
...
Я — Автор, ПРИЗНАЮ [конкретный физический симптом из Q4] и ВЫБИРАЮ [инвертированный ресурс из Q5]
...
2. НЕ ИСПОЛЬЗОВАТЬ МЕТАФОРЫ ИЗ Q3 В ФОРМУЛЕ, ТОЛЬКО ТЕЛЕСНЫЕ ОЩУЩЕНИЯ ИЗ Q4
    ...
    if len(answers) >= 4:
        ...
        for keyword in keywords:
            if keyword in q4_answer:
                ...
                symptom = q4_answer[start:end].strip()
                if symptom and not symptom[0].isupper():
                    symptom = symptom[0].upper() + symptom[1:]
                return symptom
...
def generate_fallback_report(answers: list, index: int) -> str:
    ...
    # Получаем метафору (если есть)
    metaphor = ""
    if len(answers) >= 3:
        q3_answer = answers[2]
        # Упрощаем метафору до первых 3-5 слов
        words = q3_answer.split()
        if len(words) > 5:
    ...
    if len(answers) >= 5:
        q5_answer = answers[4]
        if 'нагл' in q5_answer.lower():
            shadow_analysis = "Ваша раздражение на наглость в других людях может быть инвертировано в скрытую силу – «Право брать свое». Это означает, что вы имеете потенциал для более уверенного и самоутвержденного поведения, но пока не осознаете его."
        else:
            shadow_analysis = f"Ваша реакция на определенные качества в других указывает на подавленную часть вас самих. То, что вы запрещаете себе, является скрытым источником энергии."
...
async def send_startup_notification():
    try:
        ...
        msg = (
            ...
            f"⚙️ Версия: Обновленный отчет + нейро-ключи"
        )
        await bot.send_message(chat_id=ADMIN_ID, text=msg)
    except Exception as e:
