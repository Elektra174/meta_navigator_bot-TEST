...
QUESTIONS = [
    ...
    "Что вас больше всего бесит/раздражает в других людях? Какое качество или поведение вызывает самую сильную эмоциональную реакция?",
...
# --- ФУНКЦИЯ ЗАГРУЗКИ И ОТПРАВКИ PDF ---
...
def generate_fallback_report(answers: list, index: int) -> str:
    ...
    if len(answers) >= 6:
        q6_answer = answers[5]
        # Пытаемся понять, о каком качестве идет речь
        words = q6_answer.lower().split()
        for word in words:
            if len(word) > 4:  # Ищем слова подлиннее
                # Проверяем, содержит ли слово негативную окраску
                negative_prefixes = ['не', 'без', 'рас']
                if any(word.startswith(prefix) for prefix in negative_prefixes):
                    shadow_analysis = f"Ваше раздражение на {word} в других указывает на подавленную часть внутри Вас — человека, который может проявлять противоположное качество."
                    break
