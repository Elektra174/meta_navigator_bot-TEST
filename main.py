...
2. **Доминанта:**
   - Опиши образ из Ответа №3, используя ТОЧНО ТЕ ЖЕ СЛОВА, что и пользователь
   - Объясни, что этот образ — это сила собственного торможения
   - СЛЕДИ ЗА ГРАММАТИКОЙ: "качу" → "качу", "качаю" → "качаю" (используй как сказал пользователь)

3. **МЕТАФОРМУЛА (самое важное):**
   - Короткая императивная фраза (3-6 слов) В КАПСЛОКЕ (БОЛЬШИМИ БУКВАМИ)
   - **АЛГОРИТМ СБОРКИ: Возьми качество, которое бесит (из Ответа №5) → Преврати его в Ресурс → Направь на решение Проблемы (из Ответа №1)**
   - ИСПОЛЬЗУЙ ТОЧНЫЕ ФОРМУЛИРОВКИ ИЗ ОТВЕТОВ:
     * Если пользователь сказал "не знаю чем заняться" → используй "заняться делом/занятием"
     * Если пользователь сказал "своим заняться" → используй "своим делом"
     * НЕ ПРИДУМЫВАЙ "бизнес", если пользователь не говорил этого
   - Пример на основе ваших ответов: Если Ответ №1 "не знаю чем заняться", а Ответ №5 "бесит наглость" → Формула: "ИСПОЛЬЗУЙ НАГЛОСТЬ, ЧТОБЫ НАЙТИ ДЕЛО"
...
[Анализ образа из Ответа №3, используя ТОЧНЫЕ слова пользователя. Объясни, что это сила собственного торможения].
...
---
## 🔑 Ваша Метаформула
### [ЗДЕСЬ НАПИШИ ФРАЗУ-КОД КАПСЛОКОМ КРУПНО, ИСПОЛЬЗУЯ СЛОВА ПОЛЬЗОВАТЕЛЯ]

---
## ⚡ Инструкция по активации
**Нейрофизиология формулы:** [Объясни: Тень из Ответа №5 содержит энергию, необходимую для Ответа №1. Используй ТОЧНЫЕ формулировки из ответов].
**Действие:** [Призыв применить формулу прямо сейчас, используя ТОЧНЫЕ слова пользователя].
...
4. МЕТАФОРМУЛА: должна явно связывать Шаг 5 и Шаг 1, быть в КАПСЛОКЕ и ГРАММАТИЧЕСКИ ПРАВИЛЬНОЙ
5. ИСПОЛЬЗУЙ ТОЧНЫЕ СЛОВА ПОЛЬЗОВАТЕЛЯ, НЕ ПРИДУМЫВАЙ СВОИ
6. Если пользователь сказал "не знаю чем заняться" → говори "найти дело/занятие", НЕ "начать бизнес"
7. Если пользователь сказал "качу" → говори "качу", НЕ "качаю"
"""
...
# --- ПРАКТИКА "ВОЗВРАЩЕНИЕ СИЛЫ" С ИСПРАВЛЕННЫМИ КНОПКАМИ ---
...
async def send_practice(message: types.Message, answers: list):
    ...
    # Кнопки под практикой (ИСПРАВЛЕНО: убраны лишние кавычки)
    builder = InlineKeyboardBuilder()
    builder.row(
        types.InlineKeyboardButton(
        ...
        try:
            ...
            if hasattr(response, 'choices') and len(response.choices) > 0:
                ...
                if hasattr(choice, 'message') and hasattr(choice.message, 'content'):
                    ...
                    # Дополнительная проверка и исправление грамматики метаформулы
...
def correct_report_grammar(report: str, answers: list) -> str:
    """Исправление грамматических ошибок и адаптация к ответам пользователя"""
    import re
    
    # Собираем ключевые слова из ответов пользователя
    user_keywords = []
    for answer in answers:
        user_keywords.extend(answer.lower().split())
    ...
    # Исправляем "бизнес" на более точные формулировки если пользователь не говорил о бизнесе
    ...
    if not has_business:
        # Заменяем "бизнес" на более нейтральные варианты
        replacements = {
    ...
    # Ищем и исправляем метаформулу
    formula_pattern = r"### \[(.*?)\]"
    match = re.search(formula_pattern, report, re.IGNORECASE | re.DOTALL)
    if match:
...
def correct_formula_grammar(formula: str, answers: list) -> str:
    """Исправление грамматических ошибок в метаформуле с учетом ответов пользователя"""
    # Убираем лишние пробелы
    formula = formula.strip()
    
    # Преобразуем в капслок
    formula = formula.upper()
    
    # Исправляем типичные грамматические ошибки
    corrections = {
        "ЧТОБЫ НАЧНИ": "ЧТОБЫ НАЧАТЬ",
        "ЧТОБЫ СДЕЛАЙ": "ЧТОБЫ СДЕЛАТЬ",
        "ЧТОБЫ ИДИ": "ЧТОБЫ ИДТИ",
        "ЧТОБЫ ГОВОРИ": "ЧТОБЫ ГОВОРИТЬ",
        "ИСПОЛЬЗУЙ НАЧНИ": "ИСПОЛЬЗУЙ, ЧТОБЫ НАЧАТЬ",
        "ПРИМЕНИ НАЧНИ": "ПРИМЕНИ, ЧТОБЫ НАЧАТЬ",
        "ДЛЯ СВОЕГО БИЗНЕСА": "ДЛЯ СВОЕГО ДЕЛА",  # Заменяем бизнес на дело
        "НАЧАТЬ БИЗНЕС": "НАЧАТЬ СВОЕ ДЕЛО",
    ...
    # Если пользователь говорил "не знаю чем заняться", корректируем формулу
    if "не знаю чем заняться" in " ".join(answers).lower():
        if "ДЕЛО" not in formula and "ЗАНЯТИЕ" not in formula:
            formula = formula.replace("БИЗНЕС", "ДЕЛО")
            formula = formula.replace("НАЧАТЬ", "НАЙТИ ДЕЛО")
