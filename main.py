...
# ИЛИ используйте альтернативный хостинг:
# GUIDE_URL = "https://your-direct-link-to-pdf.com/reviziaguide.pdf"
...
ВАЖНО:
1. ФОРМУЛА ДОЛЖНА БЫТЬ ПОЛНОЙ И ЗАВЕРШЕННОЙ ФРАЗОЙ (3-5 СЛОВ)
2. НЕ ДУБЛИРОВАТЬ РАЗДЕЛ ФОРМУЛЫ
3. НЕ ПИСАТЬ ДОПОЛНИТЕЛЬНЫХ ПРИЗЫВОВ ИЛИ ТЕКСТА ПОСЛЕ "🚀 СЛЕДУЮЩИЙ ШАГ"
"""
            ...
            try:
                ...
                if report:
                    ...
                    await message.answer(
                        ...
                        f"🔑 Ваша Метаформула:\n"
                        f"Я ВЫХОЖУ ИЗ РЕЖИМА АВТОПИЛОТА\n\n"
                        f"🚀 Следующий шаг:\n"
                        f"Изучите гайд «Ревизия Маршрута» для получения полной инструкции."
                    )
                    await send_offer_buttons(message)
...
async def send_offer_buttons(message: types.Message):
    ...
    offer_text = (
        ...
        "2. 🎬 **Мастер класс «Сдвиг оптики»** — полная методика от Александра Лазаренко"
    )
...
async def handle_download_guide(callback: types.CallbackQuery):
    ...
    try:
        # Способ 1: Пробуем отправить как документ напрямую
        try:
            ...
            # Способ 2: Пробуем отправить как URL с описанием
            try:
                await callback.message.answer(
                    "📥 Гайд «Ревизия маршрута»\n\n"
                    "Ваш путеводитель к состоянию Автора жизни с помощью Метаформулы.\n\n"
                    ...
                    f"{GUIDE_URL}\n\n"
                    "Нажмите на ссылку и в правом верхнем углу выберите «Скачать».",
                    disable_web_page_preview=False
                )
                return  # Успешно отправили ссылку
            except Exception as url_error:
                logger.warning(f"Не удалось отправить ссылку: {url_error}")
                
                # Способ 3: Пробуем альтернативный формат ссылки
                try:
                    # Если используется GitHub raw, пробуем другой формат
                    if "raw.githubusercontent.com" in GUIDE_URL:
                        download_url = GUIDE_URL.replace("raw.githubusercontent.com", "github.com").replace("/raw/", "/blob/")
                        await callback.message.answer(
                            "📥 Гайд «Ревизия маршрута»\n\n"
                            "Для скачивания перейдите по ссылке и нажмите кнопку «Download»:\n"
                            f"{download_url}"
                        )
                    else:
                        # Если все способы не сработали
                        raise Exception("Все способы отправки не сработали")
                        
                except Exception as alt_error:
                    logger.error(f"Альтернативный метод тоже не сработал: {alt_error}")
                    raise Exception("Не удалось отправить гайд")
                    
    except Exception as e:
        logger.error(f"Ошибка отправки гайда: {e}")
        try:
            await callback.message.answer(
                "❌ Не удалось отправить гайд автоматически.\n\n"
                "Пожалуйста, скачайте гайд вручную по этой ссылке:\n"
                "https://github.com/Elektra174/meta_navigator_bot/blob/main/reviziaguide.pdf\n\n"
                "Нажмите на ссылку, затем кнопку «Download» в правом верхнем углу."
            )
        except:
            pass
...
def postprocess_report(report: str, answers: list) -> str:
    ...
    try:
        ...
        # Исправляем незаконченную формулу
        if "ВАША ФОРМУЛА: \"Я ВЫХОЖУ ИЗ" in report and not "Я ВЫХОЖУ ИЗ" in report[report.find("Я ВЫХОЖУ ИЗ")+10:report.find("Я ВЫХОЖУ ИЗ")+20]:
            report = re.sub(
                r'ВАША ФОРМУЛА: "Я ВЫХОЖУ ИЗ.*?["\n]',
                'ВАША ФОРМУЛА: "Я ВЫХОЖУ ИЗ РЕЖИМА АВТОПИЛОТА"',
                report
            )
        
        # Проверяем и исправляем формулу
        formula_pattern = r'🔑 ВАША МЕТАФОРМУЛА.*?\n(.+?)(?=\n\n|\n🚀|\n🎯|$)'
        match = re.search(formula_pattern, report, re.DOTALL | re.IGNORECASE)
        
        if match:
            formula = match.group(1).strip()
            if len(formula.split()) < 2 or "Я ВЫХОЖУ ИЗ" in formula and len(formula) < 20:
                complete_formula = "Я ВЫХОЖУ ИЗ РЕЖИМА АВТОПИЛОТА"
                report = report.replace(formula, complete_formula)
        
        # Убираем дублирование раздела формулы
        if report.count("🔑 ВАША МЕТАФОРМУЛА") > 1:
            parts = report.split("🔑 ВАША МЕТАФОРМУЛА")
            if len(parts) > 2:
                report = parts[0] + "🔑 ВАША МЕТАФОРМУЛА" + parts[1]
        
        # Убираем текст после "🚀 СЛЕДУЮЩИЙ ШАГ"
        if "🚀 СЛЕДУЮЩИЙ ШАГ" in report:
            next_step_index = report.find("🚀 СЛЕДУЮЩИЙ ШАГ")
            end_of_report = report[next_step_index:]
            if "\n\n" in end_of_report:
                end_index = end_of_report.find("\n\n")
                if end_index != -1:
                    report = report[:next_step_index + end_index]
...
🔑 ВАША МЕТАФОРМУЛА (КОД АКТИВАЦИИ)
Я ВЫХОЖУ ИЗ РЕЖИМА АВТОПИЛОТА

🚀 СЛЕДУЮЩИЙ ШАГ
Ваша Метаформула — это ключ зажигания. Изучите гайд «Ревизия Маршрута», чтобы получить инженерные инструкции по внедрению этой формулы."""
        except Exception as e:
