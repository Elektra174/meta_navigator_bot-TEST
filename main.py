...
QUESTIONS = [
    ...
    "Опишите ваш обычный день — «день сурка». Как проходит ваше утро, день, вечер? Что в этой рутине вас больше всего истощает?",
    ...
    "Какую цену вы платите за сохранение текущего положения? Что уходит: время, деньги, здоровье, отношения? Что и в каком объеме уходит прямо сейчас?",
    ...
    try:
        ...
        if next_step < len(QUESTIONS):
            ...
            if report:
                ...
                await message.answer(
                    "⚠️ *Не удалось сгенерировать нейрокогнитивный отчет*\n\n"
        ...
        await message.answer("⚠️ Технический сбой в когнитивном модуле. Пожалуйста, перезапустите бота командой /start")
...
def sanitize_markdown(text: str) -> str:
    ...
    # Убираем лишние обратные слэши
    text = re.sub(r'\\([#*_`\[\]()<>])', r'\1', text)
    
    # Оставляем только необходимые символы форматирования
    lines = text.split('\n')
    result_lines = []
    
    for line in lines:
        # Сохраняем эмодзи и заголовки
