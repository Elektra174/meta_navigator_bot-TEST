...
ВАЖНЫЕ ПРАВИЛА:
1. ОБРАЩЕНИЕ НА «ВЫ» ВО ВСЕМ ОТЧЕТЕ
2. ИСПОЛЬЗУЙ ТОЧНЫЕ ФОРМУЛИРОВКИ ИЗ ОТВЕТОВ ПОЛЬЗОВАТЕЛЯ
3. НЕ ПРИДУМЫВАЙ ТОГО, ЧЕГО НЕТ В ОТВЕТАХ
4. СЛЕДИ ЗА ГРАММАТИКОЙ И СТИЛЕМ
5. Метаформула должна быть грамматически правильной на русском языке
...
2. **Доминанта:**
   - Переформулируй образ из Ответа №3 в грамотном русском языке, сохраняя суть
   - Пример: "Это как бнг по кругу, стадион и бегаю по кольцу" → "Это бег по кругу на стадионе — движение без продвижения"
...
3. **МЕТАФОРМУЛА (самое важное):**
   ...
   - ИСПОЛЬЗУЙ ТОЧНЫЕ ФОРМУЛИРОВКИ ИЗ ОТВЕТОВ:
     * Если пользователь сказал "не знаю чем заняться" → используй "найти дело/занятие"
     * Если пользователь сказал "своим заняться" → используй "свое дело"
...
[Переформулируй образ из Ответа №3 в грамотном русском, объясни что это сила собственного торможения. ОБРАЩАЙСЯ НА «ВЫ»].

---
## ⚙️ Функция / Дефолт-система
[Анализ Ответа №2 и №4. Как именно ВЫ тратите нейронный ресурс на поддержание проблемы. ОБРАЩАЙСЯ НА «ВЫ»].
...
КРИТИЧЕСКИЕ ПРАВИЛА:
1. ОБРАЩЕНИЕ: строго на «Вы» во всем отчете
2. ТОН: научный, инженерный, без эзотерики
3. ТЕРМИНЫ: «нейронный ресурс», «доминанта», «субъектность», «дефолт-система»
4. ФОРМАТ: только Markdown заголовки (#, ##, ###)
5. МЕТАФОРМУЛА: должна явно связывать Шаг 5 и Шаг 1, быть в КАПСЛОКЕ и ГРАММАТИЧЕСКИ ПРАВИЛЬНОЙ
6. ИСПОЛЬЗУЙ ТОЧНЫЕ СЛОВА ПОЛЬЗОВАТЕЛЯ, НЕ ПРИДУМЫВАЙ СВОИ
7. Если пользователь сказал "не знаю чем заняться" → говори "найти дело/занятие", НЕ "начать бизнес"
8. ПЕРЕФОРМУЛИРУЙ неграмотные фразы пользователя в грамотный русский язык, сохраняя смысл
9. В разделе «Доминанта» и «Функция / Дефолт-система» обращайся на «Вы»
"""
...
def correct_report_grammar(report: str, answers: list) -> str:
    ...
    # Исправляем "человек" на "Вы"
    report = re.sub(r'\bчеловек\b', 'Вы', report, flags=re.IGNORECASE)
    report = re.sub(r'\bЧеловек\b', 'Вы', report)
    
    # Исправляем "он/она" на "Вы"
    report = re.sub(r'\bон\b', 'Вы', report, flags=re.IGNORECASE)
    report = re.sub(r'\bона\b', 'Вы', report, flags=re.IGNORECASE)
    
    # Исправляем "свое" на "свое" если было "свое"
    if "своё" in " ".join(answers).lower():
        report = re.sub(r'\bсвое\b', 'своё', report, flags=re.IGNORECASE)
    
    # Исправляем типичные грамматические ошибки в Доминанте
    if "бегу по кругу" in report or "бегаю по кольцу" in report:
        # Ищем секцию Доминанты
        dominant_start = report.find("## 🧲 Ваша Доминанта")
        if dominant_start != -1:
            dominant_end = report.find("---", dominant_start + 20)
            if dominant_end != -1:
                dominant_section = report[dominant_start:dominant_end]
                
                # Создаем корректную формулировку
                corrected_dominant = "## 🧲 Ваша Доминанта\n(Внутренний магнит застоя)\n"
                
                # Анализируем ответ пользователя и создаем грамотную формулировку
                if "стадион" in " ".join(answers).lower() and "круг" in " ".join(answers).lower():
                    corrected_dominant += "Это бег по кругу на стадионе — движение без продвижения. Этот образ представляет собой силу собственного торможения, которая не позволяет Вам найти выход и двигаться вперед."
                elif "качу" in " ".join(answers).lower() and "булыжник" in " ".join(answers).lower():
                    corrected_dominant += "Это булыжник, который Вы катите, но не можете сдвинуть с места. Этот образ представляет собой силу собственного торможения, которая не позволяет Вам двигаться вперед."
                else:
                    # Общая корректная формулировка
                    corrected_dominant += "Это образ, который символизирует Ваше текущее состояние застоя. Он представляет собой силу собственного торможения, которая не позволяет Вам двигаться вперед."
                
                # Заменяем старую секцию на исправленную
                report = report[:dominant_start] + corrected_dominant + "\n---" + report[dominant_end+3:]
    
    # Исправляем формулировки в Функции / Дефолт-системе
    if "Человек тратит" in report or "человек тратит" in report:
        report = re.sub(r'[Чч]еловек тратит', 'Вы тратите', report)
    
    if "человек сомневается" in report or "Человек сомневается" in report:
        report = re.sub(r'[Чч]еловек сомневается', 'Вы сомневаетесь', report)
