...
async def handle_download_guide(callback: types.CallbackQuery):
    ...
    try:
        # Отправляем PDF файл напрямую по URL
        ...
        # Если не работает через URL, попробуем альтернативный метод
        try:
            await callback.message.answer(
                "📥 Чтобы скачать гайд «Ревизия маршрута», перейдите по ссылке:\n"
                f"{GUIDE_URL}"
            )
...
def postprocess_report(report: str, answers: list) -> str:
    ...
    try:
        ...
        if "ВАША ФОРМУЛА: \"Я ВЫХОЖУ ИЗ" in report and not "Я ВЫХОЖУ ИЗ" in report[report.find("Я ВЫХОЖУ ИЗ")+10:report.find("Я ВЫХОЖУ ИЗ")+20]:
            # Находим и исправляем незаконченную формулу
            report = re.sub(
        ...
        if report.count("🔑 ВАША МЕТАФОРМУЛА") > 1:
            # Находим первый и последний разделы
            parts = report.split("🔑 ВАША МЕТАФОРМУЛА")
            if len(parts) > 2:
                # Оставляем только первый раздел
        ...
        if "🚀 СЛЕДУЮЩИЙ ШАГ" in report:
            ...
            # Находим конец раздела "Следующий шаг"
            if "\n\n" in end_of_report:
